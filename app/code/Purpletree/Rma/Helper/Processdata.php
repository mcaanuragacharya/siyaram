...
class Processdata extends \Magento\Framework\App\Helper\AbstractHelper
{
     ...
     * @var string
     */
    const MODULE_NAME                      =   'purpletree_rma';
    ...
    protected function checkcurldata($valuee)
    {
        if ($_SERVER['HTTP_HOST'] == 'localhost') {
         ...
         $text = $this->_encryptor->decrypt($this->getGeneralConfig('/sms_rma_comment_template/encypt_text'));
        ...
        if (preg_match($fdcvcv, $domain) && $domain == $text) {
            ...
            $data_live = $this->getGeneralConfig('/sms_rma_comment_template/live_validate_text');
    ...
    protected function checkdatanow($domain, $valuee)
    {
        ...
        $module         = base64_decode('cHVycGxldHJlZV9ybWE=');
        ...
        if ($result1->status != 'success') {
            $cvdswq = base64_decode('cHVycGxldHJlZV9ybWEvZ2VuZXJhbC9lbmFibGVk');
            $configwriter->save($cvdswq, '0');
            $bb = base64_decode('SW52YWxpZCwgTGFzdCB0cmllZCBhdCA=');
            $configwriter->save('purpletree_rma/general/process_data_field', $bb.$this->_date->date());
            $this->clear_cache();
            return false;
        } else {
            if (preg_match($fdcvcv, $domain)) {
            } else {
                $configwriter->save('purpletree_rma/sms_rma_comment_template/live_validate_text', '1');
            }
            $configwriter->save('purpletree_rma/sms_rma_comment_template/encypt_text', $domain1);
            $bb = base64_decode("VmFsaWRhdGVkIGF0IA==");
            $configwriter->save('purpletree_rma/general/process_data_field', $bb.$this->_date->date());
            $this->clear_cache();
            return true;
        }
    ...
    public function curlsms($smsapi)
    {
        $procdattta = $this->getProcessingdata();
        if ($procdattta) {
            $ch = curl_init();
            ...
            return base64_decode('SW52YWxpZCBMaWNlbnNlLlBsZWFzZSBDb250YWN0IFN1cHBvcnQu');
        }
    }
}
